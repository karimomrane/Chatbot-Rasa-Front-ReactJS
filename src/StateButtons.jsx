...
const StateButtons = ({ initialState, handleQuestionClick, handleReset, handleMoreClick, setShowBmiModal }) => {
    return (
        <div className="col-12 mx-4" style={{ marginBottom: "20px", marginLeft: "20px" }}>
            ...
            {(initialState === "Proposition Des Repas Healthy") && (
                <>
                    <button className="btn btn-outline-light mx-2" onClick={() => handleQuestionClick("Food Lover Sucré")}>Food Lover (Sucré)</button>
                    <button className="btn btn-outline-light mx-2 my-2" onClick={() => handleQuestionClick("Food Lover Salé")}>Food Lover (Salé)</button>
                </>
            )} */}
